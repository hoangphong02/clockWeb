...
import { WrapperInfo, WrapperInputNumber, WrapperLeft, WrapperRight } from "./style";
...
const OrderPage =()=> {
  ...
  const handleOnchangeDetails = (e) => {
    ...
    useEffect(()=>{
      if(state?.numcheck){
        if(!listChecked.includes(state?.numcheck)){
          setListChecked([...listChecked, state?.numcheck])
        }
      }
    },[state?.numcheck])
  ...
  useEffect(()=>{
    ...
    return ( 
        ...
        <div style={{display:"flex"}} >
            <WrapperLeft>
              <div style={{background: "rgb(240, 248, 255)",border: "1px solid rgb(194, 225, 255)",padding: "15px",borderRadius: "20px",margin:"10px 0"}}>
                <StepComponent current={currentDelivery} items={itemsDelivery} />
              </div>
            <div style={{display:"flex", gap:"50px", padding:"10px 10px",background:"#fff", margin:"0 0 10px 0"}}>
                <span style={{width:"250px"}} >
                <Checkbox onChange={handleCheckAll} checked={listChecked?.length === order?.orderItems?.length}/>
                <span>Tất cả (<span>{order?.orderItems?.length}</span> sản phẩm )</span>
                </span>
            ...
            <div style={{display:"flex", justifyContent:"center", alignItems:"center",gap:"50px",background:"#fff", padding:"10px",flexDirection:"column"}}>
                {order?.orderItems.map((order) =>{
                  if(order?.name){
                    return (                      
                             <div className="products" style={{display:"flex",width:"100%",gap:"50px"}}>
                <span style={{width:"250px",display:"flex",alignItems:"center"}}>
                    <Checkbox value={order?.product} onChange={onChange} checked={listChecked.includes(order?.product)}/>
            ...
            <span>
            <ButtonComponent style={{padding:"0"}} icon={<MinusOutlined style={{color:"#000"}}/>} onClick={()=>handleDecrease(order?.product,order?.amount)}/>
            <WrapperInputNumber min={1} max={order?.orderItems?.countInStock}  defaultValue={order?.amount} value={order?.amount} />
            <ButtonComponent style={{padding:"0"}} icon={<PlusOutlined style={{color:"#000", textAlign:"center"}}/> } onClick ={()=>handleIncrease(order?.product,order?.amount,order?.countInStock)}/>
                    </span>
            <span style={{color:"red"}}>{(order?.amount * order?.price)?.toLocaleString()} VND</span>
            <DeleteOutlined  onClick={()=> handleRemoveOrder(order?.product)}/> 
                </div>
                </div>
                    )
                  }
            ...
            <WrapperInfo style={{marginBottom:"10px"}}>
              <span>Địa chỉ:</span>
              <span style={{fontWeight:"600"}}> {`${user?.address} - ${user?.city}`}  <span style={{color:"blue",cursor:"pointer"}} onClick={handleChangeAddress}> Thay đổi</span></span>
            ...
            <WrapperInfo>
                ...
                <span>
                    <span style={{display:"flex",justifyContent:"space-between"}}>
                        <div style={{borderTop:"1px solid #333", display:"flex",justifyContent:"space-between",width:"100%"}}>
                    <span>Tổng tiền</span>
                    <p>
